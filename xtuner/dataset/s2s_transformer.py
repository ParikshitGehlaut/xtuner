...
from xtuner.utils import IGNORE_INDEX, S2S_TRANSFORMER_AUDIO_TOKEN_INDEX
...
def prepare_inputs_labels_for_S2S_Transformer(
        llm: PreTrainedModel,
    ...
    for batch_idx, cur_input_ids in enumerate(input_ids):
        num_audios = (cur_input_ids == S2S_TRANSFORMER_AUDIO_TOKEN_INDEX).sum()
        if num_audios == 0:
        ...
        audio_token_indices = [-1] + torch.where(
            cur_input_ids == S2S_TRANSFORMER_AUDIO_TOKEN_INDEX)[0].tolist() + [
...
class S2S_TRANSFORMER_Dataset(Dataset):
    """Dataset for S2S_TRANSFORMER.
